    ...
    socket.on(
        ...
        async ({
          ...
          try {
            ...
            let query = `
              SELECT b.*, s.Name as SubjectName, p.Name as ProfessorName
              FROM Block b
                     JOIN SubjectsProfessors sp ON sp.SubjectFK = b.SubjectFK
                     JOIN Subjects s ON s.Id = b.SubjectFK
                     JOIN People p ON p.Id = sp.PeopleFK
              WHERE sp.PeopleFK = ?
                AND EXISTS (
                SELECT 1
                FROM Schedule sch
                WHERE sch.Id = b.ScheduleFK
                  AND sch.StartDate <= ?
                  AND sch.EndDate >= ?
              )
            `;
            ...
            if (excludeBlockId) {
              ...
              console.log("Excluding block ID from professor check:", excludeBlockId);
            }
    ...
    socket.on(
        ...
        async ({
          ...
          try {
            ...
            // 1. Buscar blocos da BD para a sala (excluding current block if editing)
            let query = `
        SELECT * FROM Block
        WHERE ClassroomFK = ?
          AND EXISTS (
      ...
      `;
            ...
            let queryParams = [roomId, scheduleEndDate, scheduleStartDate];
